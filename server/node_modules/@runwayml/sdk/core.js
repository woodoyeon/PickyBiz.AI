...
class APIClient {
    ...
    stringifyQuery(query) {
        return Object.entries(query)
            ...
            throw new error_1.RunwayMLError(`Cannot stringify type ${typeof value}; Expected string, number, boolean, or null. If you need to pass nested query parameters, you can manually encode them, e.g. { query: { 'foo[key1]': value1, 'foo[key2]': value2 } }, and please open a GitHub issue requesting better support for your use case.`);
        })
            .join('&');
    }
...
class AbstractPage {
    ...
    async getNextPage() {
        ...
        if (!nextInfo) {
            throw new error_1.RunwayMLError('No next page expected; please check `.hasNextPage()` before calling `.getNextPage()`.');
        }
...
const validatePositiveInteger = (name, n) => {
    if (typeof n !== 'number' || !Number.isInteger(n)) {
        throw new error_1.RunwayMLError(`${name} must be an integer`);
    }
    if (n < 0) {
        throw new error_1.RunwayMLError(`${name} must be a positive integer`);
    }
    return n;
};
...
const ensurePresent = (value) => {
    if (value == null)
        throw new error_1.RunwayMLError(`Expected a value to be given but received ${value} instead.`);
    return value;
};
...
const coerceInteger = (value) => {
    ...
    throw new error_1.RunwayMLError(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
const toBase64 = (str) => {
    ...
    throw new error_1.RunwayMLError('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
